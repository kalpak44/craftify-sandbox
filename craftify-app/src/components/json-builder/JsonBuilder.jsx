...
function getDefaultField(type = "string") {
  ...
  if (type === "array") return { type, required: false, item: getDefaultField(), validations: { notEmpty: false }, description: "" };
  ...
  if (type === "string") {
    return (
      <div className="flex gap-4 items-center text-sm text-gray-300 mt-1">
        <label>Min Length: <input type="number" className="w-20 h-9 bg-gray-900 border border-gray-700 rounded px-2 text-base" value={validations.minLength || 0} min={0} onChange={disabled ? undefined : e => onChange({ ...validations, minLength: Number(e.target.value) })} disabled={disabled} /></label>
        <label>Max Length: <input type="number" className="w-20 h-9 bg-gray-900 border border-gray-700 rounded px-2 text-base" value={validations.maxLength || 255} min={1} onChange={disabled ? undefined : e => onChange({ ...validations, maxLength: Number(e.target.value) })} disabled={disabled} /></label>
        <label>Regex: <input type="text" className="w-40 h-9 bg-gray-900 border border-gray-700 rounded px-2 text-base" value={validations.regex || ""} onChange={disabled ? undefined : e => onChange({ ...validations, regex: e.target.value })} disabled={disabled} /></label>
      </div>
    );
  }
  if (type === "number") {
    return (
      <div className="flex gap-4 items-center text-sm text-gray-300 mt-1">
        <label>Min: <input type="number" className="w-20 h-9 bg-gray-900 border border-gray-700 rounded px-2 text-base" value={validations.min || 0} onChange={disabled ? undefined : e => onChange({ ...validations, min: Number(e.target.value) })} disabled={disabled} /></label>
        <label>Max: <input type="number" className="w-20 h-9 bg-gray-900 border border-gray-700 rounded px-2 text-base" value={validations.max || 100} onChange={disabled ? undefined : e => onChange({ ...validations, max: Number(e.target.value) })} disabled={disabled} /></label>
      </div>
    );
  }
  return null;
...
function EditableField({
  ...
  // Handler for property key change (input only)
  ...
  // Commit property key change on blur or Enter
    ...
    <div className="flex flex-col gap-2 mb-4 ml-4 border-l-2 border-gray-700 pl-4 py-2">
      <div className="flex items-center gap-4 mb-1">
        ...
        <select
          value={type}
          disabled={!editable}
          onChange={editable ? e => onChange(fieldId, { ...getDefaultField(e.target.value), propertyKey: keyInput, required, description, id: fieldId }) : undefined}
        ...
        {!isSystem && (
          <button onClick={onRemove} className="text-red-400 ml-4 px-3 py-2 rounded text-base h-10">Remove</button>
        )}
      </div>
...
export default function JsonBuilder({ value, onChange, showSystemFields = true }) {
  // Debug: log the value
  // console.log('JsonBuilder value:', value);
  let schema = { ...value };
  if (showSystemFields) {
  ...
  return (
    <div className="bg-gray-800 p-8 rounded">
      ...
      <div className="mb-4 flex items-center justify-between">
        ...
        <button className="bg-blue-700 text-white rounded px-4 py-2 text-base h-10" onClick={handleAddField}>+ Add field</button>
      </div>
      {customFields.length === 0 && <div className="text-gray-400 text-base ml-4">No custom fields yet.</div>}
      {customFields.map(({ id, propertyKey, field, origKey }) => (
        <EditableField
          key={id}
          fieldId={id}
