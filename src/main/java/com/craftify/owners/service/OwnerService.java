package com.craftify.owners.service;

import com.craftify.owners.document.OwnerDocument;
import com.craftify.owners.dto.OwnerDto;
import com.craftify.owners.repository.OwnerRepository;
import com.craftify.shared.exception.ApiException;
import com.craftify.shared.service.CrudServiceAbstract;
import org.springframework.stereotype.Service;

@Service
public class OwnerService extends CrudServiceAbstract<OwnerDocument, OwnerDto, String> {
  public OwnerService(OwnerRepository repository) {
    super(repository);
  }

  @Override
  protected OwnerDto toDto(OwnerDocument entity) throws ApiException {
    final var ownerDto = new OwnerDto();
    ownerDto.setName(entity.getName());
    ownerDto.setId(ownerDto.getId());
    return ownerDto;
  }

  @Override
  protected OwnerDocument toEntity(OwnerDto dto) throws ApiException {
    final var owner = new OwnerDocument();
    owner.setId(dto.getId());
    owner.setName(dto.getName());
    return owner;
  }
}
