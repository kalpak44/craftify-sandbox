...
public class FlowExecutionService {
  ...
  public Flow executeFlow(Flow flow) {
    ...
    FlowExecutionHistory history =
        flowExecutionHistoryService.createExecutionHistory(
            flow.getId(), flow.getUserId(), flow.getName(), flow.getConfiguration());
    ...
    try {
          ...
          nodeMap.keySet().stream()
              .filter(
                  id -> {
                    ...
                    boolean isTrigger =
                        "manualTrigger".equals(nodeType) || "cronTrigger".equals(nodeType);
      ...
      if (!ignoredNodes.isEmpty()) {
        ...
        executionLogs.append(
            String.format(
                "[%s] Ignoring %d unconnected nodes: %s\n",
      ...
      messagingTemplate.convertAndSend(
          "/topic/flow-execution/" + flow.getId(),
  ...
  private int traverseAndExecuteWithEvents(
      String nodeId,
      ...
      Set<String> visitedFailure,
      boolean cameFromSuccess,
    ...
    String nodeType = node.get("type").asText();
    if (!"manualTrigger".equals(nodeType)
        && !"cronTrigger".equals(nodeType)
        && !"action".equals(nodeType)) {
    ...
    // Debug logging
    logger.info(
        "Node {} execution result: success={}, using graph: {}",
        nodeId,
        nodeSucceeded,
        nodeSucceeded ? "success" : "failure");
    logger.info(
        "Node {} children from {} graph: {}",
        nodeId,
        nodeSucceeded ? "success" : "failure",
        children);
  ...
  private boolean executeActionNode(JsonNode node, StringBuilder executionLogs, String flowId) {
    try {
      ...
      String dockerImage =
          data.has("dockerImage") ? data.get("dockerImage").asText() : "kalpak44/job-runner:latest";
      String command =
          data.has("command") ? data.get("command").asText() : "print('No command provided')";
      ...
      executionLogs.append(
          String.format("[%s] Executing action node: %s\n", Instant.now(), nodeId));
      ...
      logger.info(
          "Executing action node {} with docker image: {}, command: {}",
          nodeId,
          dockerImage,
          command);
      ...
      JobExecutionResult result =
          monitorJobExecutionWithLogs(jobName, Duration.ofSeconds(timeout), flowId, nodeId);
      ...
      executionLogs.append(
          String.format(
              "[%s] Node %s completed with status: %s (exit code: %d, time: %dms)\n",
      ...
      String finalStatus =
          result.status.equalsIgnoreCase("SUCCESS") ? "Node succeeded." : "Node failed.";
      messagingTemplate.convertAndSend(
          "/topic/flow-execution/" + flowId,
      ...
      logger.info(
          "Node {} execution completed - exitCode: {}, status: {}, isSuccess: {}",
          nodeId,
          result.exitCode,
          result.status,
          isSuccess);
      ...
      String nodeId = node.get("id").asText();
      executionLogs.append(
          String.format("[%s] Node %s failed: %s\n", Instant.now(), nodeId, e.getMessage()));
  ...
  private String createCustomJob(String dockerImage, String command, String code, int timeout) {
    String jobName =
        "flow-action-" + System.currentTimeMillis() + "-" + (int) (Math.random() * 1000);
  ...
  private JobExecutionResult monitorJobExecutionWithLogs(
      String jobName, Duration timeout, String flowId, String nodeId) {
  ...
  private String getPodLogs(String podName) {
    try {
      return kubernetesClient.pods().inNamespace(NAMESPACE).withName(podName).getLog(true).trim();
    } catch (Exception e) {
