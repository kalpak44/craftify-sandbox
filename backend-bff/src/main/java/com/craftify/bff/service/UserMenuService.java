...
public class UserMenuService {
    ...
    @Autowired
    public UserMenuService(UserMenuRepository userMenuRepository, AuthentificationService authentificationService) {
    ...
    public List<UserMenu> saveMenus(List<UserMenu> menus, String userId) {
        for (UserMenu menu : menus) {
            userMenuRepository.findByUserIdAndRoute(userId, menu.route())
                    .ifPresent(existing -> {
                        throw new IllegalArgumentException("This route is already used for this user: " + menu.route());
                    });
        }
        List<UserMenu> toSave = menus.stream()
                .map(menu -> new UserMenu(
                        null,
                        userId,
                        menu.menuPath(),
                        menu.menuId(),
        ...
        try {
            ...
            throw new IllegalArgumentException("One or more menu routes are already taken for this user.");
        }
    }
    ...
    public MenuTreeNode buildMenuTree(List<UserMenu> menus) {
        ...
        for (UserMenu menu : menus) {
            ...
            for (UserMenu.MenuPathElement segment : path) {
                Optional<MenuTreeNode> existingChild = current.children().stream()
                ...
                if (existingChild.isPresent()) {
                    ...
                    MenuTreeNode newNode = new MenuTreeNode(segment.id(), segment.label(), new ArrayList<>(), null);
                    current.children().add(newNode);
                    current = newNode;
                }
            }
            // Attach the menu at the leaf
            MenuTreeNode leafNode = new MenuTreeNode(
                    current.id(),
                    current.label(),
                    current.children(),
                    menu
            );
            // Replace the last child with the leafNode containing the menu (preserve children if necessary)
    ...
    private MenuTreeNode findParent(MenuTreeNode root, List<UserMenu.MenuPathElement> path) {
        ...
        for (int i = 0; i < path.size() - 1; i++) {
            ...
            Optional<MenuTreeNode> child = current.children().stream()
                    .filter(c -> c.id().equals(segmentId))
                    .findFirst();
