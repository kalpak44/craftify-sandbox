...
public class FunctionService {
  ...
  public Page<FunctionDto> getAllFunctionsForCurrentUser(Pageable pageable, String userId) {
    return repository
        ...
        .map(fn -> new FunctionDto(fn.id(), fn.name(), fn.status(), fn.executionMode()));
  }

  public void saveFunction(
      String functionName, String functionType, String userId, String commitHash) {
    Instant now = Instant.now();
    repository.save(
        new FunctionRegistration(
            null, userId, functionName, "ACTIVE", functionType, commitHash, now));
  }
